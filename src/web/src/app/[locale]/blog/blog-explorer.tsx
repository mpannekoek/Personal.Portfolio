...
import type { BlogPreviewCard } from "../../../lib/blog";

type SortOption = "date-desc" | "date-asc" | "title-asc";
type ViewMode = "metro" | "text";

export type BlogExplorerLabels = {
    empty: string;
    searchLabel: string;
...
type BlogExplorerProps = {
    ...
    labels: BlogExplorerLabels;
};

export default function BlogExplorer({ posts, labels }: BlogExplorerProps) {
    ...
    return (
        <>
            ...
            <div className="mt-10 flex flex-col gap-4 md:mt-12 lg:flex-row lg:items-center lg:justify-end">
                <div className="flex items-center gap-3 self-start lg:self-auto">
                    ...
                    >
                        <option value="date-desc">{labels.newestSort}</option>
                        <option value="date-asc">{labels.oldestSort}</option>
                        <option value="title-asc">{labels.titleSort}</option>
                    </select>

                    <div className="inline-flex items-center rounded-lg border border-[var(--border)] bg-[var(--surface-elevated)] p-1">
                        <button
                            type="button"
                            aria-label={labels.metroView}
            ...
            {visiblePosts.length === 0 ? (
                <p className="mt-8 text-sm text-[var(--text-soft)]">{labels.empty}</p>
            ) : null}

            {visiblePosts.length > 0 && viewMode === "metro" ? (
                <div className="mt-8 grid grid-cols-1 gap-x-10 gap-y-4 xl:grid-cols-2">
                    {visiblePosts.map((post) => (
                        <BlogListItem key={post.id} post={post} titleTag="h2" />
                    ))}
                </div>
            ) : null}

            {visiblePosts.length > 0 && viewMode === "text" ? (
